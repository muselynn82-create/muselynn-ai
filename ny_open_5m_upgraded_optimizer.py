...
RESULT_SHEET_NAME = "NY_OPEN_UPGRADE_RESULTS"
TOP_SHEET_NAME = "NY_OPEN_UPGRADE_TOP20"
TRADES_SHEET_NAME = "NY_OPEN_UPGRADE_TRADES"
RUN_LOG_SHEET_NAME = "NY_OPEN_UPGRADE_RUNLOG"
...
def get_or_create_ws(spreadsheet, title, rows=8000, cols=80):
    try:
...
def trend_allowed(row, params):
    ...
    if filt == "BEAR_OR_CRASH":
        return row["big_trend"] in ["BIG_BEAR", "BIG_CRASH"]
...
def backtest_params(df, params, collect_trades=False):
    ...
    for trade_date, day_all in grouped:
        ...
        for i in range(1, len(day_df)):
            ...
            exit_time, exit_price, exit_reason, gross_pnl = simulate_trade(
                day_df, i, entry_price, stop_price, target_price, params["max_hold_hours"]
            )
    ...
    if trades_df.empty:
        return {**params, "trades": 0, "win_rate": 0, "total_return": 0, "max_drawdown": 0, "avg_win": 0,
                "avg_loss": 0, "profit_factor": 0, "tp_count": 0, "sl_count": 0, "max_hold_count": 0,
                "time_exit_count": 0}, trades_df
...
def main():
    print("NY Open 5M Upgraded Optimizer started:", now_kst(), flush=True)

    spreadsheet = init_gspread()
    result_ws = get_or_create_ws(spreadsheet, RESULT_SHEET_NAME, rows=15000, cols=80)
    ...
    for k in ["risk_reward", "atr_target_mult", "retest_tolerance", "min_range_pct", "max_range_pct",
              "min_volume_ratio", "min_range_atr_mult"]:
    ...
    print("NY Open 5M Upgraded Optimizer finished:", now_kst(), flush=True)
