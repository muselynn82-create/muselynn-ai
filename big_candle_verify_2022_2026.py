...
# ============================================================
# BTC BIG CANDLE BREAKOUT VERIFY 2022-2026
# - No optimization
# - Applies the best BTC train parameter directly to OOS period
# ============================================================
...
RESULT_SHEET_NAME = "BTC_BIG_CANDLE_VERIFY_RESULTS"
TRADES_SHEET_NAME = "BTC_BIG_CANDLE_VERIFY_TRADES"
RUN_LOG_SHEET_NAME = "BTC_BIG_CANDLE_VERIFY_RUNLOG"

CACHE_PREFIX = "btc_big_candle_verify_2022_2026"
...
def simulate_long_with_unrealized_mdd(df, entry_idx, entry_price, stop_price, target_price, current_equity):
    ...
    for j in range(entry_idx + 1, len(df)):
        ...
        local_min_equity = min(local_min_equity, current_equity * (1 + unrealized_low_net / 100))
...
def backtest_params(df, params, collect_trades=False):
    ...
    while i < len(df):
        ...
        exit_idx, exit_time, exit_price, exit_reason, gross_pnl, local_min_equity = simulate_long_with_unrealized_mdd(
            df, i, entry_price, stop_price, target_price, equity
        )

        max_drawdown = min(max_drawdown, ((local_min_equity - peak_equity) / peak_equity) * 100)
    ...
    if trades_df.empty:
        return {**params, "trades": 0, "win_rate": 0, "total_return": 0, "cagr": 0, "max_drawdown": 0,
                "avg_win": 0, "avg_loss": 0, "profit_factor": 0, "tp_count": 0, "sl_count": 0,
...
def main():
    print("BTC Big Candle VERIFY 2022-2026 started:", now_kst(), flush=True)
    ...
    append_run_log(log_ws, "Verify started")
    df = load_data(PARAMS["symbol"], PARAMS["interval"])

    stats, trades_df = backtest_params(df, PARAMS, collect_trades=True)
    stats["run_time"] = now_kst()

    results_df = pd.DataFrame([stats]).replace([float("inf"), float("-inf")], "").fillna("")
    trades_df = trades_df.replace([float("inf"), float("-inf")], "").fillna("")
    ...
    append_run_log(log_ws, "Verify finished")
    print("BTC Big Candle VERIFY 2022-2026 finished:", now_kst(), flush=True)
