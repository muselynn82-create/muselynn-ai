...
RESULT_SHEET_NAME = "OPTIMIZER_RESULTS"
TOP_SHEET_NAME = "OPTIMIZER_TOP20"
RUN_LOG_SHEET_NAME = "OPTIMIZER_RUN_LOG"

# 거래량 기반 구조 연구: 기존 딥풀백 vs 거래량 폭발 반전
# 총 조합: 864개
PARAM_GRID = {
    "strategy_type": ["DEEP_PULLBACK", "VOLUME_REVERSAL"],
    "entry_score": [70, 80],
    "rsi_limit": [24, 26, 28],
    "volume_mult": [2.0, 2.5],
    "take_profit": [1.5, 1.8, 2.2],
    "stop_loss": [-1.0, -1.2],
    "trail_start": [1.2, 1.5],
...
def get_or_create_ws(spreadsheet, title, rows=6000, cols=60):
    try:
...
def score_deep_pullback(now, params):
    price = now["close"]
    score = 0
    if (
    ...
    if now["ema20"] > now["ema50"]:
        score += 15
    if now["volume"] > now["volume_ma"] * params["volume_mult"]:
        score += 20
    return score


def score_volume_reversal(now, params):
    price = now["close"]
    score = 0
    if now["rsi"] < params["rsi_limit"]:
        score += 25
    if now["volume"] > now["volume_ma"] * params["volume_mult"]:
        score += 35
    if now["close_position"] >= 0.70:
        score += 25
    if now["close"] > now["open"]:
        score += 15
    if now["low"] <= now["bb_lower"] * 1.003:
        score += 20
    if price > now["ema100"]:
        score += 15
    return score
...
def calculate_score(now, params):
    if params["strategy_type"] == "DEEP_PULLBACK":
        return score_deep_pullback(now, params)
    if params["strategy_type"] == "VOLUME_REVERSAL":
        return score_volume_reversal(now, params)
    return 0
...
def main():
    ...
    result_ws = get_or_create_ws(spreadsheet, RESULT_SHEET_NAME, rows=6000, cols=60)
    ...
    append_run_log(log_ws, "Volume research optimizer started")
    ...
    for idx, values in enumerate(combos, start=1):
        ...
        stats = run_backtest(df_15m, df_1h, df_4h, params)
    ...
    append_run_log(log_ws, "Volume research optimizer finished")
