...
RESULT_SHEET_NAME = "HA_4H_BTC_OPT_RESULTS"
TOP_SHEET_NAME = "HA_4H_BTC_OPT_TOP50"
PERIOD_SHEET_NAME = "HA_4H_BTC_OPT_PERIODS"
TRADES_SHEET_NAME = "HA_4H_BTC_OPT_TRADES"
RUN_LOG_SHEET_NAME = "HA_4H_BTC_OPT_RUNLOG"

CACHE_PREFIX = "ha_4h_btc_opt"
...
PARAM_GRID = {
    # HA + Stoch RSI V2
    # BTCUSDT 4H 전용 파라미터 재탐색
    #
    # 목적:
    # 1) 4시간봉 보조전략을 BTC 전용으로 확정하기 전 TP/SL/EMA거리/보유시간만 제한적으로 재탐색
    # 2) ETH/SOL은 이전 검증에서 제외
    # 3) 진입 논리는 그대로 유지하고 청산/필터 강도만 확인
    #
    # 고정:
    #   symbol = BTCUSDT
    #   interval = 4h
    ...
    #   TP 2.5~4.5
    #   SL -1.2~-2.4
    #   EMA distance 0.3~1.0
    #   max_hold 24~96

    "symbol": SYMBOLS,
    "interval": ["4h"],
    "direction_mode": ["LONG_ONLY"],
    "take_profit": [2.5, 3.0, 3.5, 4.0, 4.5],
    "stop_loss": [-1.2, -1.5, -1.8, -2.1, -2.4],
    "stoch_zone": ["NONE"],
    "wick_tolerance": [0.0003],
    "ema_price_source": ["REAL_CLOSE"],
    "min_ema_distance_pct": [0.3, 0.5, 0.7, 1.0],
...
def backtest_params(df, params, collect_trades=False):
    ...
    if trades_df.empty:
        return {**params, "trades": 0, "win_rate": 0, "total_return": 0, "cagr": 0, "max_drawdown": 0, "avg_win": 0,
...
PERIODS = [
    ...
    ("2024_2026", "2024-01-01", "2026-05-25"),
    ("2022_2026", "2022-01-01", "2026-05-25"),
    ("ALL_2017_2026", "2017-01-01", "2026-05-25"),
]


def calc_cagr_by_dates(total_return_pct, start_date, end_date):
    start_dt = datetime.strptime(start_date, "%Y-%m-%d")
    end_dt = datetime.strptime(end_date, "%Y-%m-%d")
    years = max((end_dt - start_dt).days / 365.25, 0.01)
...
def summarize_trade_slice(trades_df, params, period_name, start_date, end_date):
    base = {
        **params,
        "period": period_name,
    ...
    df = df[(df["entry_dt"] >= start_dt) & (df["entry_dt"] < end_dt)].copy()
    ...
    for pnl in df["net_pnl"].astype(float):
        equity *= (1 + pnl / 100)
        peak = max(peak, equity)
        max_dd = min(max_dd, ((equity - peak) / peak) * 100)
        equities.append(equity)

    wins = df[df["net_pnl"].astype(float) > 0]
    losses = df[df["net_pnl"].astype(float) <= 0]
    exit_counts = df["exit_reason"].value_counts().to_dict()

    total_return = equity - 100
    gross_profit = wins["net_pnl"].astype(float).sum() if not wins.empty else 0
    gross_loss = losses["net_pnl"].astype(float).sum() if not losses.empty else 0
    pf = 999.0 if gross_loss == 0 else abs(gross_profit / gross_loss)
    ...
    return {
        **base,
        ...
        "total_return": round(total_return, 2),
        "cagr": calc_cagr_by_dates(total_return, start_date, end_date),
        "max_drawdown": round(max_dd, 2),
...
def build_period_rows(trades_df, params):
    rows = []
    for period_name, start_date, end_date in PERIODS:
        rows.append(summarize_trade_slice(trades_df, params, period_name, start_date, end_date))
    return rows
...
def score_rank(row):
    ...
    score += pf * 120
    score += float(row["total_return"]) * 1.6
    score += float(row.get("cagr", 0)) * 3.0
    score += float(row["win_rate"]) * 0.6
    score += float(row["max_drawdown"]) * 5
...
def main():
    print("HA 4H BTC Optimize Fee 0.15 started:", now_kst(), flush=True)
    ...
    period_ws = get_or_create_ws(spreadsheet, PERIOD_SHEET_NAME, rows=1000, cols=40)
    ...
    for idx, values in enumerate(combos, start=1):
        ...
        period_rows.extend(build_period_rows(trades_df, params))
        ...
        if idx % 25 == 0 or idx == len(combos):
            ...
            if not temp_df.empty:
                ...
                clear_and_write(
                    top_ws,
                    list(temp_df.head(50).columns),
                    temp_df.head(50).astype(str).values.tolist(),
                )

                temp_period_df = pd.DataFrame(period_rows)
                if not temp_period_df.empty:
                    temp_period_df = temp_period_df.replace([float("inf"), float("-inf")], "").fillna("")
                    clear_and_write(
                        period_ws,
                        list(temp_period_df.columns),
                        temp_period_df.astype(str).values.tolist(),
                    )
    ...
    if results_df.empty:
        ...
        clear_and_write(period_ws, ["message"], [["No period results"]])
    ...
    if not period_df.empty:
        period_df = period_df.replace([float("inf"), float("-inf")], "").fillna("")
        period_df = period_df.sort_values(by=["symbol", "period_start", "period"], ascending=True)
        clear_and_write(period_ws, list(period_df.columns), period_df.astype(str).values.tolist())
    else:
        clear_and_write(period_ws, ["message"], [["No period results"]])
    ...
    print("HA 4H BTC Optimize Fee 0.15 finished:", now_kst(), flush=True)
    print("Saved result to:", RESULT_SHEET_NAME, flush=True)
    print("Saved top50 to:", TOP_SHEET_NAME, flush=True)
    print("Saved period results to:", PERIOD_SHEET_NAME, flush=True)
