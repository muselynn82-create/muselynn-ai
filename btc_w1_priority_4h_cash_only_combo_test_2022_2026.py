...
# ============================================================
# BTC WEEKLY PRIORITY + 4H CASH-ONLY COMBO TEST 2022-2026
# ============================================================
# 목적:
#   1) 주봉 포지션이 있으면 주봉 100% 우선
#   2) 주봉 포지션이 없을 때만 4시간봉 100% 운용
#   3) 주봉 단독 vs 4시간봉 단독 vs 조합 전략 최종 수익률 비교
#
# 기간:
...
SUMMARY_SHEET_NAME = "BTC_W1_4H_COMBO_SUMMARY"
DETAIL_SHEET_NAME = "BTC_W1_4H_COMBO_DETAIL"
WEEKLY_TRADES_SHEET_NAME = "BTC_W1_4H_COMBO_WEEKLY_TRADES"
H4_TRADES_SHEET_NAME = "BTC_W1_4H_COMBO_4H_TRADES"
COMBO_TRADES_SHEET_NAME = "BTC_W1_4H_COMBO_TRADES"
RUN_LOG_SHEET_NAME = "BTC_W1_4H_COMBO_RUNLOG"

CACHE_PREFIX = "btc_w1_4h_combo_2022_2026"
...
def is_inside_any_weekly_position(entry_dt, exit_dt, weekly_df):
    if weekly_df.empty:
        return False, None, 0.0, 0.0

    h_duration_hours = max((exit_dt - entry_dt).total_seconds() / 3600, 0.0001)
    best = None

    for _, w in weekly_df.iterrows():
        w_entry = w["entry_dt"]
        w_exit = w["exit_dt"]
        overlap_start = max(entry_dt, w_entry)
        overlap_end = min(exit_dt, w_exit)

        if overlap_start < overlap_end:
            overlap_hours = (overlap_end - overlap_start).total_seconds() / 3600
            if best is None or overlap_hours > best["overlap_hours"]:
                best = {
    ...
    return True, best, best["overlap_hours"], best["overlap_ratio_of_h4_trade"]


def compound_return_with_mdd(trades_df):
    ...
    return round(total_return, 4), round(mdd, 4), int(len(trades_df)), round(win_rate, 2)


def build_combo_simulation(weekly_df, h4_df):
    ...
    # 주봉 거래는 조합 전략에 그대로 포함
    for _, w in weekly.iterrows():
        combo_trade_rows.append({
            "combo_source": "WEEKLY_PRIORITY",
            "trade_no": int(w["trade_no"]),
            "entry_time": w["entry_time"],
            "exit_time": w["exit_time"],
            "net_pnl": float(w["net_pnl"]),
            "exit_reason": w["exit_reason"],
            "note": "주봉 포지션 보유. 4시간봉 거래보다 우선 적용.",
        })

    # 4시간봉 거래는 주봉 포지션 밖에서 발생한 것만 조합에 포함
    for _, h in h4.iterrows():
        h_entry = h["entry_dt"]
        h_exit = h["exit_dt"]
        is_overlap, best_overlap, overlap_hours, overlap_ratio = is_inside_any_weekly_position(h_entry, h_exit, weekly)

        if is_overlap:
            status = "SKIP_H4_INSIDE_WEEKLY"
            include_combo = False
            weekly_trade_no = best_overlap["weekly_trade_no"]
            weekly_entry_time = best_overlap["weekly_entry_time"]
            weekly_exit_time = best_overlap["weekly_exit_time"]
        else:
            status = "USE_H4_OUTSIDE_WEEKLY"
            include_combo = True
            weekly_trade_no = ""
            weekly_entry_time = ""
            weekly_exit_time = ""
        ...
        detail_rows.append({
            ...
            "h4_net_pnl": float(h["net_pnl"]),
            "h4_exit_reason": h["exit_reason"],
            "combo_action": status,
            "included_in_combo": include_combo,
            "weekly_trade_no": weekly_trade_no,
            "weekly_entry_time": weekly_entry_time,
            "weekly_exit_time": weekly_exit_time,
            "overlap_hours": overlap_hours,
            "overlap_ratio_of_h4_trade": overlap_ratio,
        })

        if include_combo:
            combo_trade_rows.append({
                "combo_source": "H4_CASH_ONLY",
                "trade_no": int(h["trade_no"]),
                "entry_time": h["entry_time"],
                "exit_time": h["exit_time"],
                "net_pnl": float(h["net_pnl"]),
                "exit_reason": h["exit_reason"],
                "note": "주봉 미보유 구간에서만 4시간봉 거래 적용.",
            })

    detail_df = pd.DataFrame(detail_rows)
    combo_df = pd.DataFrame(combo_trade_rows)

    if not combo_df.empty:
        combo_df["entry_dt"] = pd.to_datetime(combo_df["entry_time"])
        combo_df = combo_df.sort_values(by=["entry_dt", "exit_time"]).reset_index(drop=True)

        # 혹시라도 같은 기간 포지션이 겹치면 먼저 진입한 거래를 우선하고 겹치는 거래는 제외
        filtered = []
        ...
        for _, row in combo_df.iterrows():
            ...
            if entry_dt >= last_exit:
                ...
                last_exit = exit_dt
        combo_df = pd.DataFrame(filtered)

    weekly_return, weekly_mdd, weekly_count, weekly_wr = compound_return_with_mdd(weekly)
    h4_return, h4_mdd, h4_count, h4_wr = compound_return_with_mdd(h4)

    inside_h4 = detail_df[detail_df["combo_action"] == "SKIP_H4_INSIDE_WEEKLY"] if not detail_df.empty else pd.DataFrame()
    outside_h4 = detail_df[detail_df["combo_action"] == "USE_H4_OUTSIDE_WEEKLY"] if not detail_df.empty else pd.DataFrame()

    inside_return, inside_mdd, inside_count, inside_wr = compound_return_with_mdd(
        inside_h4.rename(columns={"h4_net_pnl": "net_pnl"}) if not inside_h4.empty else pd.DataFrame()
    )
    outside_return, outside_mdd, outside_count, outside_wr = compound_return_with_mdd(
        outside_h4.rename(columns={"h4_net_pnl": "net_pnl"}) if not outside_h4.empty else pd.DataFrame()
    )

    combo_return, combo_mdd, combo_count, combo_wr = compound_return_with_mdd(combo_df)
    ...
    summary_rows = [
        ...
        },
        {
            ...
            "trades": h4_count,
            "win_rate": h4_wr,
            "compound_return": h4_return,
            "mdd": h4_mdd,
            "note": "4시간봉 전략만 100% 운용",
        },
        {
            "metric": "H4_INSIDE_WEEKLY_SKIPPED",
            "trades": inside_count,
            "win_rate": inside_wr,
            "compound_return": inside_return,
            "mdd": inside_mdd,
            "trade_ratio_pct": round(inside_count / h4_count * 100, 2) if h4_count else 0,
            "note": "조합전략에서 제외되는 4시간봉 거래. 주봉 보유 중 발생.",
        },
        {
            "metric": "H4_OUTSIDE_WEEKLY_USED",
            "trades": outside_count,
            "win_rate": outside_wr,
            "compound_return": outside_return,
            "mdd": outside_mdd,
            "trade_ratio_pct": round(outside_count / h4_count * 100, 2) if h4_count else 0,
            "note": "조합전략에서 사용되는 4시간봉 거래. 주봉 미보유 구간.",
        },
        {
            "metric": "COMBO_W1_PRIORITY_H4_CASH_ONLY",
            "trades": combo_count,
            "win_rate": combo_wr,
            "compound_return": combo_return,
            "mdd": combo_mdd,
            "added_return_vs_weekly": round(combo_return - weekly_return, 4),
            "note": "주봉 보유 시 주봉 우선, 주봉 미보유 시 4시간봉 운용",
        },
    ]

    if combo_return > weekly_return:
        decision = "조합전략이 주봉 단독보다 수익률 개선"
    elif combo_return == weekly_return:
        decision = "조합전략과 주봉 단독 차이 없음"
    else:
        decision = "조합전략이 주봉 단독보다 낮음"
    ...
    return pd.DataFrame(summary_rows), detail_df, combo_df

def main():
    print("BTC W1 priority + 4H cash-only combo test started:", now_kst(), flush=True)
    ...
    combo_ws = get_or_create_ws(spreadsheet, COMBO_TRADES_SHEET_NAME, rows=1000, cols=40)
    log_ws = get_or_create_ws(spreadsheet, RUN_LOG_SHEET_NAME, rows=100, cols=10)

    append_run_log(log_ws, "Combo simulation started")
    ...
    summary_df, detail_df, combo_df = build_combo_simulation(weekly_trades, h4_trades)
    ...
    combo_df = safe_sheet_df(combo_df)
    ...
    clear_and_write(
        combo_ws,
        list(combo_df.columns) if not combo_df.empty else ["message"],
        combo_df.astype(str).values.tolist() if not combo_df.empty else [["No combo trades"]],
    )

    append_run_log(log_ws, "Combo simulation finished")
    ...
    print("Saved combo trades to:", COMBO_TRADES_SHEET_NAME, flush=True)
    print("BTC W1 priority + 4H cash-only combo test finished:", now_kst(), flush=True)
